...
class FusedLinearCrossEntropy(torch.autograd.Function):
    ...
    def forward(
        ...
        if TRITON_AVAILABLE and hidden.is_cuda:
            ...
            try:
                from sense_v2.llm.kernels.triton_src import triton_fused_linear_cross_entropy_forward
    ...
    def backward(ctx, grad_output: torch.Tensor):
        ...
        if ctx.use_triton and TRITON_AVAILABLE and hidden.is_cuda:
            try:
                from sense_v2.llm.kernels.triton_src import triton_fused_linear_cross_entropy_backward
