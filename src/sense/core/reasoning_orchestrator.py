...
class ReasoningOrchestrator(BaseAgent):
    ...
    def _sanitize_input(self, user_input: str) -> str:
        ...
        2. Escape common prompt injection patterns
        3. Limit length
        4. Remove control characters

        Returns sanitized string safe for LLM prompting.
        """
        if not user_input:
            return ""
        ...
        # Escape triple quotes and code fences that might break prompt structure
...
"""
    ...
    def _get_router_prompt(self, task, memory_context=None, system_time="", system_year=""):
...
"""
    ...
    def _get_tool_mode_prompt(self, instinct_hint=None, memory_context=None, system_time="", system_year=""):
...
"""
    ...
    def _get_chat_mode_prompt(self, memory_context=None, system_time="", system_year=""):
        base = f"""
{self._get_reality_override_block(system_time, system_year)}
You are SENSE, a TRUTH-SEEKING ANALYST.
...
"""
    ...
    async def _decide_mode(self, task, memory_context=None, system_time="", system_year=""):
        try:
            response = await self.client.chat.completions.create(
                model=self.model_name,
                messages=[{"role": "user", "content": self._get_router_prompt(task, memory_context, system_time, system_year)}],
                temperature=0.0,
                max_tokens=300 
            )
    ...
    async def task_run(self, task: str):
        ...
        mode = await self._decide_mode(task, episodic_context, system_time, system_year)
        if mode == "TOOL" and self.tools:
            system_prompt = self._get_tool_mode_prompt(instinct, episodic_context, system_time, system_year)
        else:
            system_prompt = self._get_chat_mode_prompt(episodic_context, system_time, system_year)
        ...
        for turn in range(5):
            try:
                ...
                if not tool_name: return f"✅ FINAL ANSWER:\n{content}"
